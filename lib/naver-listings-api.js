...
async function getCoordFromSearch(location) {
  ...
  const isCloud = process.env.VERCEL === '1' || process.env.RENDER === 'true';
  const coordTimeout = isCloud ? 15000 : 30000;
  const retryOpts = isCloud ? { maxAttempts: 5, backoffMs: 2000 } : undefined;
  try {
...
async function getList(query) {
  ...
  return getNaverLandListFromMLand({
    ...
    retryAttempts: isCloud ? 5 : 3,
    retryBackoffMs: isCloud ? 2000 : 800,
    initialDelayMs: isCloud ? 1500 : 0,
  });
}
