...
async function getCoordFromSearch(location) {
  ...
  const coordTimeout = isServerless ? 15000 : 30000;
  const retryOpts = isServerless ? { maxAttempts: 5, backoffMs: 2000 } : undefined;
  try {
...
async function getList(query) {
  ...
  return getNaverLandListFromMLand({
    ...
    requestTimeout: isServerless ? 15000 : 30000,
    maxPages: isServerless ? 1 : 4,
    retryAttempts: isServerless ? 5 : 3,
    retryBackoffMs: isServerless ? 2000 : 800,
    initialDelayMs: isServerless ? 1500 : 0,
  });
}
