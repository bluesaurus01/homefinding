...
async function getCoordFromSearch(location) {
  ...
  const coordTimeout = process.env.VERCEL === '1' ? 8000 : 30000;
  try {
...
async function getNaverLandListFromMLand({ location, lat, lng, tradeType, count, requestTimeout, maxPages }) {
  ...
  while (results.length < count) {
    ...
    const fetchOpts = { headers: M_LAND_HEADERS, timeout, validateStatus: (s) => s === 200 };
    let res;
    try {
      res = await withRetry(() => axios.get(listUrl, fetchOpts));
    } catch (e) {
...
async function getList(query) {
  ...
  return getNaverLandListFromMLand({
    ...
    count: isServerless ? 20 : 40,
    requestTimeout: isServerless ? 8000 : 30000,
    maxPages: isServerless ? 1 : 4,
  });
}
