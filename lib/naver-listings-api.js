...
async function getNaverLandListFromMLand({ location, lat, lng, tradeType, count }) {
  ...
  while (results.length < count) {
    ...
    const fetchOpts = { headers: M_LAND_HEADERS, timeout: 15000, validateStatus: (s) => s === 200 };
    let res;
    try {
      res = await axios.get(listUrl, fetchOpts);
    } catch (e) {
      if (e.code === 'ECONNABORTED' || (e.message && e.message.includes('timeout'))) {
        try { res = await axios.get(listUrl, fetchOpts); } catch (e2) {
          const err = new Error('네이버 서버 응답 지연. 잠시 후 다시 시도해 주세요.');
